...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const loadingIndicatorGageData = document.getElementById('loading_gage_data_cda');
    loadingIndicatorGageData.style.display = 'block';

    const jsonFile = 'gage_control2.json';
    console.log('jsonFile: ', jsonFile);
    // const jsonFile = 'gage_control_testtest.json';
    ...
    basinData.forEach(basin => {
        // Iterate over the keys of each object to find location IDs
        Object.keys(basin).forEach(async key => {
            // Check if the key starts with 'loc_'
            if (key.startsWith('loc_')) {
                // Prepare variable to pass in when call api
                const locationId = basin[key].location_id;
                console.log('locationId: ', locationId);

                // Location level "Flood"
                const levelIdFlood = basin[key].level_id_flood;
                const levelIdEffectiveDateFlood = basin[key].level_id_effective_date_flood;
                const levelIdUnitIdFlood = basin[key].level_id_unit_id_flood;

                // Location level "NGVD29"
                const levelIdNgvd29 = basin[key].level_id_ngvd29;
                const levelIdEffectiveDateNgvd29 = basin[key].level_id_effective_date_ngvd29;
                const levelIdUnitIdNgvd29 = basin[key].level_id_unit_id_ngvd29;

                // Location level "Record Stage"
                const levelIdRecordStage = basin[key].level_id_record_stage;
                const levelIdEffectiveDateRecordStage = basin[key].level_id_effective_date_record_stage;
                const levelIdUnitIdRecordStage = basin[key].level_id_unit_id_record_stage;
...
function mergeData(basinData, combinedFirstData, combinedSecondData, combinedThirdData, combinedForthData, combinedFifthData, combinedSixthData) {
    ...
    // Loop through each object in basinData
    basinData.forEach(basin => {
        // Create an object to store merged data for the current basin
        let mergedBasinData = { basin: basin.basin };

        // Loop through each key in basin
        Object.keys(basin).forEach(key => {
            // Check if the key starts with 'loc_'
            if (key.startsWith('loc_')) {
                // Get the location_id from the current location
                let locationId = basin[key].location_id;

                // Find the corresponding data from combinedFirstData based on location_id
                let firstData = combinedFirstData.find(data => data.name === locationId);

                // Add firstData metadata to mergedBasinData if firstData exists, otherwise set to null
                if (firstData) {
                    // If firstData exists, merge its metadata into mergedBasinData under the current key
                    mergedBasinData[key] = {
                        ...basin[key], // Preserve existing properties of basin[key]
                        metadata: firstData // Add metadata from firstData
                    };
                } else {
                    // If no matching data is found for firstData, log a message and set metadata to null
                    console.log(`No match found for location ID '${locationId}'. Skipping merge for '${key}'.`);
                    mergedBasinData[key] = {
                        ...basin[key], // Preserve existing properties of basin[key]
                        metadata: null // Set metadata to null
                    };
                }



                // Find the corresponding data from combinedSecondData based on location_id
                let secondData = combinedSecondData.find(data =>
                    data['location-level-id'].split('.')[0] === locationId
                );

                // If secondData exists, merge the data into main json
                if (secondData) {
                    // Add flood data from secondData to mergedBasinData under the current key
                    mergedBasinData[key]['flood'] = secondData;
                } else {
                    // If no matching data is found for secondData, log a message and set flood data to null
                    console.log(`No match found for secondData`);
                    mergedBasinData[key]['flood'] = null;
                }


                // Find the corresponding data from combinedThirdData based on location_id
                let thirdData = combinedThirdData.find(data => 
                    data && data['location-id'] === locationId);
                // Add thirdData metadata to mergedBasinData if thirdData exists, otherwise set to null
                if (thirdData) {
                    mergedBasinData[key]['basin'] = thirdData;
                } else {
                    console.log(`No match found for location ID '${locationId}'. Skipping merge for '${key}'.`);
                    mergedBasinData[key]['basin'] = null;
                }



                // Find the corresponding data from combinedForthData based on location_id
                let forthData = combinedForthData.find(data => data && data['location-id'] === locationId);
                // Add forthData metadata to mergedBasinData if forthData exists, otherwise set to null
                if (forthData) {
                    mergedBasinData[key]['owner'] = forthData;
                } else {
                    console.log(`No match found for location ID '${locationId}'. Skipping merge for '${key}'.`);
                    mergedBasinData[key]['owner'] = null;
                }


                // Find the corresponding data from combinedFifthData based on location_id
                let fifthData = combinedFifthData.find(data =>
                    data['location-level-id'].split('.')[0] === locationId
                );

                // If fifthData exists, merge the data into main json
                if (fifthData) {
                    // Add flood data from fifthData to mergedBasinData under the current key
                    mergedBasinData[key]['record stage'] = fifthData;
                } else {
                    // If no matching data is found for fifthData, log a message and set flood data to null
                    console.log(`No match found for fifthData`);
                    mergedBasinData[key]['record stage'] = null;
                }

                // Find the corresponding data from combinedSixthData based on location_id
                let sixthData = combinedSixthData.find(data =>
                    data['location-level-id'].split('.')[0] === locationId
                );
                // If sixthData exists, merge the data into main json
                if (sixthData) {
                    // Add flood data from sixthData to mergedBasinData under the current key
                    mergedBasinData[key]['ngvd29'] = sixthData;
                } else {
                    // If no matching data is found for sixthData, log a message and set flood data to null
                    console.log(`No match found for sixthData`);
                    mergedBasinData[key]['ngvd29'] = null;
                }
        ...
        // Push the merged basin data into allData array if at least one match was found
        if (Object.keys(mergedBasinData).length > 1) {
            allData.push(mergedBasinData);
        }
    });
}
...
function createTable(allData) {
    console.log("createTable function is called."); // Check if the function is being called
        ...
        for (let locKey in locationData) {
            ...
            if (locKey.startsWith('loc_')) {
                ...
                if (locData.visible !== false) {
                    ...
                    fetchAndUpdateStage(stageCell, tsidStage, flood_level, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    ...
                    fetchAndUpdateFlow(flowCell, locData.tsid_flow_coe, locData.tsid_flow_coe_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateFlow(flowCell, locData.tsid_flow_usgs, locData.tsid_flow_usgs_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateFlow(flowCell, locData.tsid_flow_nws, locData.tsid_flow_nws_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateFlow(flowCell, locData.tsid_flow_mvr, locData.tsid_flow_mvr_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateFlow(flowCell, locData.tsid_flow_slope, locData.tsid_flow_slope_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    

                    // PRECIP
                    const precipCell = row.insertCell();
                    fetchAndUpdatePrecip(precipCell, locData.tsid_precip_raw, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    ...
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water, locData.tsid_temp_water_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water2, locData.tsid_temp_water2_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water3, locData.tsid_temp_water3_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water4, locData.tsid_temp_water4_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water5, locData.tsid_temp_water5_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water6, locData.tsid_temp_water6_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water7, locData.tsid_temp_water7_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water8, locData.tsid_temp_water8_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water9, locData.tsid_temp_water9_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water10, locData.tsid_temp_water10_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water11, locData.tsid_temp_water11_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water12, locData.tsid_temp_water12_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water13, locData.tsid_temp_water13_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water14, locData.tsid_temp_water14_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water15, locData.tsid_temp_water15_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_water16, locData.tsid_temp_water16_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_air, locData.tsid_temp_air_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_air2, locData.tsid_temp_air2_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_air_max, locData.tsid_temp_air_max_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_temp_air_min, locData.tsid_temp_air_min_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do, locData.tsid_do_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do2, locData.tsid_do2_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do3, locData.tsid_do3_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do4, locData.tsid_do4_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do5, locData.tsid_do5_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do6, locData.tsid_do6_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do7, locData.tsid_do7_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do8, locData.tsid_do8_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do9, locData.tsid_do9_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do10, locData.tsid_do10_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do11, locData.tsid_do11_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do12, locData.tsid_do12_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do13, locData.tsid_do13_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do14, locData.tsid_do14_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_do15, locData.tsid_do15_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_cond, locData.tsid_cond_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_cond2, locData.tsid_cond2_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth, locData.tsid_depth_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth2, locData.tsid_depth2_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth3, locData.tsid_depth3_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth4, locData.tsid_depth4_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth5, locData.tsid_depth5_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth6, locData.tsid_depth6_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth7, locData.tsid_depth7_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth8, locData.tsid_depth8_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth9, locData.tsid_depth9_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth10, locData.tsid_depth10_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth11, locData.tsid_depth11_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth12, locData.tsid_depth12_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth13, locData.tsid_depth13_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth14, locData.tsid_depth14_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_depth15, locData.tsid_depth15_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_ph, locData.tsid_ph_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_turb, locData.tsid_turb_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_speed_wind, locData.tsid_speed_wind_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_speed, locData.tsid_speed_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_pressure, locData.tsid_pressure_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);

                    fetchAndUpdateWaterQuality(waterQualityCell, locData.tsid_dir_wind, locData.tsid_dir_wind_label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count);
                    ...
                    // riverMileCell.innerHTML = (parseFloat(locData.metadata.elevation)).toFixed(2);
                    riverMileCell.innerHTML = "-cdana-";
...
function fetchAndUpdateStage(stageCell, tsidStage, flood_level, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count) {
    if (tsidStage !== null) {
            ...
            .then(nws3Days => {
                ...
                if (nws3Days !== null) {
                    innerHTMLStage  = "<table id='nws'>"
                                    ...
                                    + "</tr>"
                                    + "<tr>"
                                        + "<td colspan='3' class='day_nws_ded' title='Data Entry Date'>" + "Forecast Date: " + "-cdana-" + "</td>";
                                    + "</tr>"
                                    +"<table>";
                } else {
                    innerHTMLStage  = "<span class='missing'>"
...
// Function to get flows data
function fetchAndUpdateFlow(flowCell, tsidFlow, label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count) {
    if (tsidFlow !== null) {
        let urlFlow = null;
        if (cda === "public") {
            ...
            .then(flow => {
                ...
                if (lastNonNullFlowValue === null) {
                    ...
                    innerHTMLFlow = "<span class='last_max_value' title='" + flow.name + ", Value = " + roundedValueFlowLast + ", Date Time = " + timestampFlowLast + "'>"
                                    + "<a href='https://wm.mvs.ds.usace.army.mil/web_apps/plot_macro/public/plot_macro.php?cwms_ts_id=" + flow.name + "&start_day=4&end_day=0' target='_blank'>"
...
// Function to get flows data
function fetchAndUpdatePrecip(precipCell, tsid, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count) {
    if (tsid !== null) {
        // Fetch the time series data from the API using the determined query string
        let urlPrecip = null;
        if (cda === "public") {
            ...
            .then(precip => {
                ...
                if (lastNonNullPrecipValue === null) {
                    ...
                    innerHTMLPrecip =   "<table id='precip'>"
                                        ...
                                        + "<a href='../../../web_apps/plot_macro/public/plot_macro.php?cwms_ts_id=" + precip.name + "&start_day=4&end_day=0' target='_blank'>"
                                            + valuePrecipLast
                                            + "</a>"
                                        + "</span>"
                                        + " "
...
// Function to get flows data
function fetchAndUpdateWaterQuality(waterQualityCell, tsid, label, currentDateTimeMinus2Hours, currentDateTime, currentDateTimeMinus30Hours, c_count) {
    if (tsid !== null) {
        // Fetch the time series data from the API using the determined query string
            ...
            .then(waterQuality => {
                ...
                if (lastNonNullWaterQualityValue === null) {
                    ...
                    innerHTMLWaterQuality = "<span class='last_max_value' title='" + waterQuality.name + ", Value = " + valueWaterQualityLast + ", Date Time = " + timestampWaterQualityLast + "'>"
                                    + "<a href='https://wm.mvs.ds.usace.army.mil/web_apps/plot_macro/public/plot_macro.php?cwms_ts_id=" + waterQuality.name + "&start_day=4&end_day=0' target='_blank'>"
